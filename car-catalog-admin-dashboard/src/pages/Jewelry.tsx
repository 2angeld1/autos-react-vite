...
import { Loader2, Plus, Search, Edit2, Trash2, Filter, DollarSign, Package, AlertTriangle, CheckCircle2, Gem } from 'lucide-react';
import { motion } from 'framer-motion';
import Button from '@/components/common/Button';
import Modal from '@/components/common/Modal';
import JewelryProductForm from '@/components/jewelry/JewelryProductForm';
import { useJewelry } from '@/hooks/pages/useJewelry';
import { fadeIn, slideUp, staggerContainer, scaleIn } from '@/animations/variants';

const Jewelry: React.FC = () => {
    const { state, actions } = useJewelry();
    const {
        ...
        <motion.div initial="hidden" animate="visible" variants={fadeIn} className="min-h-screen bg-gray-50 p-6">
                ...
                <div className="flex items-center justify-between">
                    <div>
                        <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100 flex items-center gap-3">
                            <motion.div
                                variants={scaleIn}
                                className="p-2 bg-gradient-to-br from-pink-500 to-rose-600 rounded-xl text-white shadow-lg shadow-pink-500/20"
                            >
                                <Gem className="h-7 w-7" />
                            </motion.div>
                            LuxJewel
                        </h1>
                        <p className="text-gray-500 dark:text-gray-400 mt-1">Gestiona anillos, collares, relojes y alta joyería</p>
                    </div>
                    <Button
                        variant="primary"
                        icon={<Plus className="h-5 w-5" />}
                        onClick={actions.handleAddProduct}
                        className="!bg-pink-600 hover:!bg-pink-700 shadow-md shadow-pink-500/20"
                    >
                        Agregar Joya
                    </Button>
                </div>
            ...
            <motion.div variants={staggerContainer} className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                <div className="bg-white dark:bg-gray-800 rounded-xl p-5 shadow-sm border border-gray-100 dark:border-gray-700">
                    <div className="flex items-center justify-between">
                        <div>
                            <p className="text-sm text-gray-500 dark:text-gray-400">Total Piezas</p>
                            <p className="text-2xl font-bold text-gray-900 dark:text-white">{stats.totalProducts}</p>
                        </div>
                        <div className="p-3 bg-pink-50 dark:bg-pink-900/20 rounded-lg">
                            <Gem className="h-6 w-6 text-pink-600 dark:text-pink-400" />
                        </div>
                    </div>
                </div>
            ...
            <motion.div variants={slideUp} className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-4 mb-6">
                <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
                    <div className="flex-1 relative w-full">
                        ...
                        <input
                            type="text"
                            placeholder="Buscar joyas LuxJewel..."
                            value={searchTerm}
                            onChange={(e) => actions.setSearchTerm(e.target.value)}
                            className="w-full pl-10 pr-4 py-2.5 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg focus:ring-2 focus:ring-pink-500 focus:border-transparent text-gray-900 dark:text-white placeholder-gray-400"
                        />
                    </div>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left">
                        <thead className="bg-gray-50 dark:bg-gray-900/50 border-b border-gray-100 dark:border-gray-700">
                            <tr>
                                <th className="px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Pieza</th>
                        ...
                        <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                            {filteredProducts.map(product => (
                                <tr key={product._id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                        ...
                                        <div className="flex items-center gap-3">
                                            <div className="w-12 h-12 bg-gray-100 dark:bg-gray-700 rounded-lg flex items-center justify-center overflow-hidden border">
                                                {product.thumbnail ? (
                                                    ...
                                                    <Gem className="h-6 w-6 text-pink-400" />
                                                )}
                                            </div>
                                            <div>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <span className="px-3 py-1 bg-pink-50 dark:bg-pink-900/30 text-pink-700 dark:text-pink-300 rounded-full text-xs font-medium border border-pink-100">
                                            {product.specs?.jewelryCategory || 'N/A'}
                                        </span>
                                    </td>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        {product.stock === 0 ? (
                                            <span className="flex items-center gap-1.5 text-red-600 text-sm font-medium"><Package className="h-4 w-4" /> Agotada</span>
                                        ) : product.stock < 5 ? (
            ...
            {filteredProducts.length === 0 && !loading && (
                <motion.div variants={slideUp} className="text-center py-12">
                    <Gem className="h-16 w-16 text-pink-300 mx-auto mb-4" />
                    <h3 className="text-lg font-medium text-gray-900">No hay joyas en LuxJewel</h3>
                    <p className="text-gray-500">Agrega tu primera pieza de alta joyería para comenzar</p>
                </motion.div>
            )}
                ...
                <div className="flex justify-center gap-2 mt-6">
                    {Array.from({ length: pagination.totalPages }, (_, i) => i + 1).map(page => (
                        <button
                            ...
                            className={`w-10 h-10 rounded-lg font-medium transition-colors ${pagination.page === page ? 'bg-pink-600 text-white' : 'bg-white text-gray-600 hover:bg-gray-100 border'}`}
                        >
                            {page}
                        </button>
                    ))}
            ...
            <Modal
                ...
                size="md"
                title={editingProduct ? 'Editar Joya LuxJewel' : 'Nueva Joya LuxJewel'}
            >
                <JewelryProductForm
